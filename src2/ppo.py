...
class PPO(PolicyGradient):
    ...
    def update_policy(self, observations, actions, advantages, old_logprobs):
        ...
        r_ts = torch.exp(logprobs) / torch.exp(old_logprobs)
        clipped_r_ts = torch.clip(r_ts, 1 - self.eps_clip, 1 + self.eps_clip)
        loss = -torch.mean(torch.minimum(r_ts * advantages, clipped_r_ts * advantages))
    ...
    def train(self):
        ...
        torch.save(self.policy.network.state_dict(), "ppo_rand.pth")
    ...
    def sample_path(self, env, num_episodes=None):
        ...
        while num_episodes or t < self.config.batch_size:
            ...
            for step in range(self.config.max_ep_len):
                ...
                # action, old_logprob = self.policy.act(states[-1], return_log_prob = True)
                # action, old_logprob = action, old_logprob
