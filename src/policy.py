...
class CategoricalPolicy(BasePolicy, nn.Module):
    ...
    def action_distribution(self, observations):
        ...
        for i in range(logits.shape[0]):
            # pdb.set_trace()
            updated_obs = (observations[i, 1:]).to(torch.int)
            if 0 in updated_obs:
                index_of_first_zero = np.where(updated_obs == 0)[0][0]
                updated_obs = updated_obs[:index_of_first_zero]
            updated_obs = updated_obs[updated_obs > observations[i, 0]]
            mask[i, updated_obs] = True
            mask[i, 0] = True
